...
**比對嚴格度分三層**（中日韓關鍵字一律用子字串，因為中日韓沒有詞邊界）：

| 欄位 | 純英數關鍵字的規則 | 效果 |
|------|-------------------|------|
| 歌名／頻道 | **詞首**（前面不接英數字） | 搜 `monster` 仍命中 `Monsters`；但 `Hebe` 不再命中頻道 `OnlyTheBestOST`、`live` 不再命中 `Alive` |
| 標籤 | **整詞**（前後都不接英數字） | 搜 `ai` 不會命中標籤 `Aimer`；`아이유(IU)` 仍命中（括號即邊界）|

**標籤灌水防護**：標籤數超過 `MAX_TRUSTED_TAGS`（25）的影片整支不採信其標籤。
實測全庫 2,356 部的標籤數中位數只有 4、P95 為 24，但有影片掛到 82 個、塞滿一整排
他人藝名（SEO）。門檻 25 **不影響任何一部官方「- Topic」影片**——那正是母語名的來源。
